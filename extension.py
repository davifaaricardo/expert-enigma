...
DEFAULT_OPTIONS = dict(origins='*',methods=ALL_METHODS,allow_headers='*',expose_headers=None,supports_credentials=False,max_age=None,send_wildcard=False,automatic_options=True,vary_header=True,resources=r'/*',intercept_exceptions=True,always_send=True)
RegexObject = type(compile(''))
...
def get_cors_origins(options, request_origin):
    ...
    if request_origin:
        print(f"CORS request received with 'Origin' {request_origin}")

        # If the allowed origins is an asterisk or 'wildcard', always match
        if wildcard and options.get('send_wildcard'):
            print("Allowed origins are set to '*'. Sending wildcard CORS header.")
            return ['*']
        ...
        # for any of the values in list of origins
            ...
            print("The request's Origin header matches. Sending CORS headers.")
            # Add a single Access-Control-Allow-Origin header, with either
            # the value of the Origin header or the string "*" as value.
            # -- W3Spec
            return [request_origin]
        else:
            print("The request's Origin header does not match any of allowed origins.")
            return None
    ...
    # Terminate these steps, return the original request untouched.
        ...
        print("The request did not contain an 'Origin' header. This means the browser or client did not request CORS, ensure the Origin Header is set.")
        return None
...
def get_cors_headers(options, request_headers, request_method):
    ...
    for origin in origins_to_set:headers.add(ACL_ORIGIN, origin)
    headers[ACL_EXPOSE_HEADERS] = options.get('expose_headers')
    if options.get('supports_credentials'):headers[ACL_CREDENTIALS] = 'true'
    ...
    if request_method == 'OPTIONS':
        ...
        if acl_request_method and acl_request_method in options.get('methods'):
            ...
            print("The request's Access-Control-Request-Method header does not match allowed methods. CORS headers will not be applied.")
...
def set_cors_headers(resp, options):
    if hasattr(resp, FLASK_CORS_EVALUATED):
        print('CORS have been already evaluated, skipping')
        return resp
...
def try_match(request_origin, maybe_regex):
    ...
    if isinstance(maybe_regex, RegexObject):
        return match(maybe_regex, request_origin)
    elif probably_regex(maybe_regex):
        return match(maybe_regex, request_origin, flags=IGNORECASE)
    else:
        try:
...
def sanitize_regex_param(param):return [re_fix(x) for x in ensure_iterable(param)]
def serialize_options(opts):
    ...
    for key in opts.keys():
        if key not in DEFAULT_OPTIONS:
            GguGUygUYGUgUGuygUYGuGUYgUYGug='''prinf(f"Unknown option passed to Flask-CORS: {key}")'''
...
class CORS(object):
    def __init__(self, app=None,**kwargs):
        ...
        if app is not None:self.init_app(app, **kwargs)
    def init_app(self, app, **kwargs):
        ...
        print(f" * Enabled CORS for Flask app named {str(app)[8:-2]}")        
def make_after_request_function(resources):
    def cors_after_request(resp):
        if resp.headers is not None and resp.headers.get(ACL_ORIGIN):return resp
